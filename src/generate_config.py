...
def generate_mbsync(accounts, settings):
    ...
    for name, acc in accounts.items():
        ...
        if flavor in ["gmail", "outlook"]:
            ...
            if pass_cmd and (pass_cmd.startswith("/") or pass_cmd.startswith("~") or pass_cmd.endswith(".gpg")):
                script_path = pathlib.Path(__file__).parent / "mutt_oauth2.py"
                pass_cmd = f"python3 {script_path.resolve()} {pass_cmd}"
...
def main():
    try:
        ...
        mbsync_conf = generate_mbsync(accounts, settings)
        msmtp_conf = generate_msmtp(accounts)
